...
async def create_news(news_in: NewsCreate, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_verified_author)):
    ...
    celery_app.send_task(
        "app.tasks.email.notify_new_news",
        args=[news.id],
        queue="email",
    )
...
async def read_news(news_id: int, db: AsyncSession = Depends(get_db)):
    ...
    if cached:
        _logger.info("cache_hit news_id=%s", news_id)
        return cached
    ...
    item = NewsOut.model_validate(n).model_dump(mode='json')
    await set_json(f"news:{news_id}", item, ttl_seconds=_NEWS_TTL)
    _logger.info("cache_miss news_id=%s", news_id)
    return item

@router.get("/", response_model=List[NewsOut])
async def list_news(skip: int = 0, limit: int = 100, db: AsyncSession = Depends(get_db)):
    key = f"news:list:{skip}:{limit}"
    cached = await get_json(key)
    if cached:
        _logger.info("cache_hit list skip=%s limit=%s", skip, limit)
        return cached
    result = await db.execute(
        select(News).options(selectinload(News.author)).offset(skip).limit(limit)
    )
    items = [NewsOut.model_validate(n).model_dump(mode='json') for n in result.scalars().all()]
    await set_json(key, items, ttl_seconds=_NEWS_TTL)
    _logger.info("cache_miss list skip=%s limit=%s", skip, limit)
    return items
